...
public class Root extends User{
	...
	public Root(String name,String password)  {
		...
		while(true)
		{
			System.out.println("\t\t欢迎您，超级管理员"+name+"使用本系统");
			System.out.println("\t\t=============");
			...
			System.out.println("\t\t8.查看所有用户列表");
			System.out.println("\t\t9.修改用户权限");
	...
	public void GiveGrant() {
		...
		System.out.print("\t\t请输入您要更改权限的用户id:");
		int id = sc.nextInt();
		try {
			DoMain.console.dispResultSet(DoMain.conn.createStatement().executeQuery("Select * From users "
					+ "Where user_id = "+id+" ;"));
		} catch (SQLException e) {
			System.out.println("搜索失败，请返回重试");
			return;
		}
		System.out.print("\t\t您是否要修改此用户       Y/N");
		...
		System.out.print("将此用户更改为什么权限？（root,admin,ordinary）");
		String newGrant = sc.next();
		if(newGrant.equals("root")||newGrant.equals("admin")||newGrant.equals("ordinary")){
			try {
				DoMain.conn.createStatement().executeUpdate("Update users Set user_grant = '"+newGrant+"' where user_id = "+id+";");
			} catch (SQLException e) {
				System.out.println("搜索失败，请返回重试");
				return;
			}	
